...
class DetailViewController: UIViewController,UITableViewDataSource,UITableViewDelegate {
    let dyCellIdentifier = "dyIdentifier"
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        mTableView.estimatedRowHeight = 239
        mTableView.rowHeight = UITableViewAutomaticDimension
        
        mTableView.backgroundColor = UIColor.darkGrayColor()
    }
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        var cell = cellCache[String(indexPath.row)] as? StoryBoardCell
        if((cell == nil)){
...
//        
//        let cell = self.getCellFromIndexPath(indexPath) as StoryBoardCell
//        cellCache[String(indexPath.row)] = cell
////        let height = cell.heightForCell()
...
//    }
    ...
    func getCellFromIndexPath(indexPath:NSIndexPath)->StoryBoardCell{
        
        var cell = self.mTableView.dequeueReusableCellWithIdentifier(dyCellIdentifier) as? StoryBoardCell
        if(cell == nil){
            cell = StoryBoardCell(style:.Subtitle, reuseIdentifier: dyCellIdentifier)
        }
